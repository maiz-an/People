...
$back_url = ($_SESSION['user_type'] === 'organization') ? 'organizationHome.php' : 'home.php';
...
// if ($user_id > 0) {
//     $role_sql = "SELECT user_type FROM login WHERE id = ?"; 
...
//         $user_role = $role_result->fetch_assoc()['user_type']; 
//     }
// }
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['report'])) {
    ...
    if ($report_category) { // Check if report_category is not null
        ...
        if ($stmt->execute()) {
            ...
            $reporter_sql = "SELECT id, name FROM login WHERE id = ?";
            ...
            $reporter_data = $reporter_result->fetch_assoc();

            // Fetch food item and poster details
            $food_sql = "SELECT free_food.food_title, free_food.id AS food_id, free_food.report_category, 
                                login.id AS poster_id, login.name AS poster_name
                         FROM free_food 
            ...
            // Fetch admin email
            $admin_sql = "SELECT email FROM admin WHERE user_type = 'admin' LIMIT 1";
            $admin_result = $conn->query($admin_sql);
            $admin_email = $admin_result->fetch_assoc()['email'];

            // Prepare email details
            $reporter_name = htmlspecialchars($_SESSION['name']); // Reporter name from the session
            $reporter_id = intval($_SESSION['id']);
            $food_title = $food_data['food_title'];
            $poster_id = $food_data['poster_id'];
            $poster_name = $food_data['poster_name'];
            $report_category = $food_data['report_category'];
            ...
            try {
                ...
                $mail->SMTPAuth = true;
                $mail->Username = 'info.people.platfrom@gmail.com'; // Your SMTP email
                $mail->Password = 'xtvqpbrsbtmnbnhv'; // Your SMTP password
                ...
                $mail->isHTML(true);
                $mail->Subject = "Community Resources Reported: $food_title";
                $mail->Body = "
                    <p>Hello Admin,</p>
                    <p>A food item has been reported on the platform. Below are the details:</p>
                    <ul>
                        <li><strong>Food ID:</strong> {$food_id}</li>
                        <li><strong>Food Title:</strong> {$food_title}</li>
                        <li><strong>Poster ID:</strong> {$poster_id}</li>
                        <li><strong>Poster Name:</strong> {$poster_name}</li>
                        <li><strong>Report Category:</strong> {$report_category}</li>
                        <li><strong>Reporter ID:</strong> {$reporter_id}</li>
                        <li><strong>Reporter Name:</strong> {$reporter_name}</li>
                    </ul>
                    <p>Please take the necessary actions.</p>
                    <p>Best regards,<br>The People Platform Team</p>
                ";

                $mail->send();
                $msg = "Report submitted successfully. An email has been sent to the admin.";
            } catch (Exception $e) {
                $msg = "Report submitted successfully, but the email could not be sent.";
            }
        } else {
...
$user_name = htmlspecialchars($_SESSION['name'] ?? "Unknown");
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<div class="container">
    <div class="header">
    <a href="<?php echo $back_url; ?>" class="back"><img src="SiteIcons/arrow-w.png" alt="arrow-w" ></a>
