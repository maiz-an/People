...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $quantity_value = isset($_POST['quantity']) ? mysqli_real_escape_string($conn, $_POST['quantity']) : '';
    $measurement = isset($_POST['measurement']) ? mysqli_real_escape_string($conn, $_POST['measurement']) : '';
    ...
    // Combine quantity and measurement into a single string
    $quantity = $quantity_value . ' ' . $measurement;
    ...
    if ($nearby_users_result && mysqli_num_rows($nearby_users_result) > 0) {
        while ($user = mysqli_fetch_assoc($nearby_users_result)) {
            ...
            try {
                ...
                $mail->setFrom('no-reply@people.com', 'GiveAway near HurryUp!!');
                $mail->addAddress($user_email);

                $mail->isHTML(true);
                $mail->Subject = "New Giveaway Near You: $food_title";
                $mail->Body = "
                    <p>Hello $user_name,</p>
                    <p>A new giveaway titled '<strong>$food_title</strong>' is available near your location on the People: Community Sharing Platform.</p>
                    <p><strong>Description:</strong> $description<br>
                    <strong>Quantity:</strong> $quantity<br>
                    <strong>Pickup Time:</strong> $pickup_time</p>
                    <p>To learn more and claim this giveaway, please log in to your account.</p>
                    <p>Best regards,<br>
...
<div class="container">
    ...
    <h2><span style="color: #ff009a;">Give</span>Away</h2>
    </div>
    <div class="gap"></div>
...
<div class="form-group-radio">
    ...
    <div class="form-group-radio-ca">
        <input type="radio" id="category-food" name="category" value="food" checked onclick="updateLabels('Food')">
        <label for="category-food">Food</label>
    </div>
    <div class="form-group-radio-ca">
        <input type="radio" id="category-nonfood" name="category" value="non-food" onclick="updateLabels('Item')">
...
<div class="form-group file-upload">
    <label id="label-food-image" for="food-images">Food Image</label>
    <img id="upload-icon" src="SiteIcons/burger.png" alt="Upload" class="upload-icon" onclick="document.getElementById('food-images').click();" />
...
<div class="form-group">
    <label id="label-food-title" for="food-title">Food Title</label>
    <input type="text" id="food-title" name="food_title" placeholder="Enter the Name of GiveAway " required>
</div>

<div class="form-group">
    <label id="label-description" for="description">Description</label>
    <textarea id="description" name="description" rows="3" placeholder="Describe the GiveAway" required></textarea>
</div>

<div class="form-group">
    <label id="label-quantity" for="quantity">Quantity</label>
    <span style="display: inline;">Measurement</span>
    <select id="measurement" name="measurement">
        <option value="pieces">Number of Pieces</option>
        <option value="kg">Kilogram (kg)</option>
        <option value="liters">Liter (L)</option>
        <option value="meters">Meter (m)</option>
    </select>
    <input type="text" id="quantity" name="quantity" placeholder="Enter the quantity of GiveAway" required>
</div>


<div class="form-group">
    <label id="label-pickup-time" for="pickup-time">Pickup Time</label>
    <input type="text" id="pickup-time" name="pickup-time" placeholder="After 4pm - 10pm" required>
</div>

<div class="form-group">
    <label id="label-pickup-instruction" for="pickup-instruction">Your Pickup Instruction</label>
    <textarea id="pickup-instruction" name="pickup_instruction" rows="3" placeholder="Instruction for pickup GiveAway" required></textarea>
</div>
        ...
        <div class="form-group">
            <label for="location">
                <i class="fa fa-map-marker location-icon"></i>GiveAway Location
            </label>
...
<div id="success-modal" class="modal">
    <div class="modal-content">
        ...
        <div class="modal-buttons">
            <button class="btn-primary" onclick="addAnotherItem()">Add Another Item</button>
